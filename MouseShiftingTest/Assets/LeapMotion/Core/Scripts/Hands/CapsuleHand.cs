...
namespace Leap.Unity
{
    ...
    public class CapsuleHand : HandModelBase
    {
        ...
        public Vector3 PositionPalmUpdatedRet { get => positionPalmUpdatedRet; set => positionPalmUpdatedRet = value; }
        ...
        //JFGA
        private Vector3 positionPalmUpdatedRet;
        private Vector3 positionPalmUpdated;
        private void updatePalmPosition(Vector3 nPosition)
        {
            positionPalmUpdated = nPosition;
            GameObject targMannagerObj = GameObject.Find("Master");
            if (targMannagerObj != null)
            {
                TargetedController positionController = targMannagerObj.GetComponent<TargetedController>();
                if (positionController != null)
                    PositionPalmUpdatedRet = positionController.giveRetargetedPosition(nPosition);
                else
                    PositionPalmUpdatedRet = nPosition;
            }
        }
        private Vector3 getRPosition(Vector3 nPosition)
        {
            /*TargetedController positionController = gameObject.GetComponent<TargetedController>();
                if (positionController != null)
                    return positionController.giveRetargetedPosition(nPosition);
                else
                    return nPosition;*/
            return nPosition - positionPalmUpdated + PositionPalmUpdatedRet;
        }
        ...
        {
            updatePalmPosition(_hand.PalmPosition.ToVector3());
            SphereCollider handCollider = gameObject.GetComponent<SphereCollider>();
            if (handCollider != null)
                handCollider.transform.position = positionPalmUpdatedRet;// TO DELETE
            if (canDraw )//JFGA
            { //JFGA
                ...
                foreach (var finger in _hand.Fingers)
                {
                    for (int j = 0; j < 4; j++)
                    {
                        ...
                        Vector3 position = finger.Bone((Bone.BoneType)j).NextJoint.ToVector3(); //JFGA
                                                                                                //Vector3 position = getRPosition(finger.Bone((Bone.BoneType)j).NextJoint.ToVector3());
                        _spherePositions[key] = position;

                        //drawSphere(position);
                        drawSphere(getRPosition(position));
                    }
                }
                ...
                // Vector3 palmPosition = _hand.PalmPosition.ToVector3(); JFGA
                ...
                Vector3 thumbBaseToPalm = _spherePositions[THUMB_BASE_INDEX] - _hand.PalmPosition.ToVector3(); // JFGA
                Vector3 mockThumbJointPos = _hand.PalmPosition.ToVector3() + Vector3.Reflect(thumbBaseToPalm, _hand.Basis.xBasis.ToVector3()); //JFGA
                drawSphere(getRPosition(mockThumbJointPos));
                ...
                if (_showArm)
                {
                    ...
                    drawSphere(getRPosition(armFrontRight));
                    drawSphere(getRPosition(armFrontLeft));
                    drawSphere(armBackLeft);
                    drawSphere(armBackRight);

                    drawCylinder(getRPosition(armFrontLeft), getRPosition(armFrontRight));
                    drawCylinder(armBackLeft, armBackRight);
                    drawCylinder(getRPosition(armFrontLeft), armBackLeft);
                    drawCylinder(getRPosition(armFrontRight), armBackRight);
                }
                ...
                for (int i = 0; i < 5; i++)
                {
                    for (int j = 0; j < 3; j++)
                    {
                        ...
                        drawCylinder(getRPosition(posA), getRPosition(posB));
                    }
                }
                ...
                drawCylinder(getRPosition(mockThumbJointPos), THUMB_BASE_INDEX);
                drawCylinder(getRPosition(mockThumbJointPos), PINKY_BASE_INDEX);
            }//JFGA
        ...
        {
            drawCylinder(getRPosition(_spherePositions[a]), getRPosition(_spherePositions[b]));//JFGA
        }

        private void drawCylinder(Vector3 a, int b)
        {
            drawCylinder(a, getRPosition(_spherePositions[b])); // JFGA
        }
